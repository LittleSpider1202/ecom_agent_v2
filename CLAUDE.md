...
4. 读 docs/env-notes.md（环境事实和踩坑，避免重复试错）
5. 读 docs/arch-decisions.md（架构决策，改动相关模块前必看）
6. 运行 git log --oneline -10（了解最近提交）
7. 确认当前分支，检查是否有未提交的变更
8. 运行 init.sh 确保服务已启动
```
...
### Feature 执行节奏：批次 + 上下文检查

每次以 **5 个 feature** 为一批执行，完成一批后：

1. 将本批踩坑写入 `docs/env-notes.md` 并 commit
2. **检查上下文使用量**：
   - 剩余 > 50%（上下文使用不足一半）→ 自动拉取下一批 5 个 feature，继续执行
   - 剩余 ≤ 50% → 停止，更新 `claude-progress.txt`，提示用户开启新 session

```
完成第 N 批（5个）→ 写 env-notes → commit
  ├─ 上下文剩余 > 50% → 继续第 N+1 批
  └─ 上下文剩余 ≤ 50% → 收尾，结束 session
```

**注意**：上下文判断靠自我估算（已用 token 量 vs 总量），无法精确，保守估计即可。宁可早停，不要撑到上下文耗尽导致输出截断。
...
1. **写入前先搜索**：用关键词（如"uvicorn"、"ReactFlow"、"localStorage"）搜索 `docs/env-notes.md` 已有内容
2. **有则补充，无则新增**：找到同类主题 → 在原有条目下追加补充信息；未找到 → 新增主题章节
3. **禁止时间追加**：不要按 session 时间顺序追加，`docs/env-notes.md` 按主题组织，每个主题只有一份
...
### 每次 commit 后必须部署到小主机（强制执行）

```bash
bash deploy.sh
```

- 脚本自动完成：前端构建 → SCP 后端 → docker cp 前端 → 重启 uvicorn
- 部署后确认健康检查通过（脚本内置，失败会报错）
- 不部署就等于代码没有生效，禁止跳过
...
| 文件 | 职责 | 写入时机 | 何时加载 |
|------|------|---------|---------|
| `docs/dev-environment.md` | 服务端口、启动命令、依赖版本、环境限制 | 引入新中间件或环境变更时更新 | 启动服务、排查环境问题时 |
| `docs/env-notes.md` | 踩坑记录、失败的尝试、行为与预期不符 | 每个 feature commit 时追加 | 每次 session 启动时必读 |
| `docs/arch-decisions.md` | 架构决策、技术选型及原因 | 做出重要决策时追加 | 修改对应模块前按需读 |
